...
function ShowStore_ClientHelper()
	...
	if localPlayer ~= Game.GetLocalPlayer() then return end
		...
		if HasCosmetic(currentlySelected.data.id) then
			...
			if currentlySelected.PartOfSubscription and propAllowSubscriptionPurchase then
				if localPlayer:HasPerk(subscriptionPerkRef) then
					...
					controlsLocked = true
					while Events.BroadcastToServer("BUYCOSMETIC", currentlySelected.data.id)  == BroadcastEventResultCode.EXCEEDED_RATE_LIMIT do
						Task.Wait(0.1)
					end 
				else 
					currentlySelected = nil
