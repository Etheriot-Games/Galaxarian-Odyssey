...
function ShowStore_ServerHelper(player)
	if ABGS.GetGameState() ~= ABGS.GAME_STATE_LOBBY then return end
	if player ~= nil then
...
local function lookForGliderIndex(Id) 
	for i,v in ipairs(GLIDER_MODULE) do
		if v == Id then
			return i
		end
	end
...
function ApplyCosmetic(player, templateId, cosmeticId, visible)
	...
	if type(cosmeticId) == "string" then
		cosmeticId = lookForGliderIndex(cosmeticId)
	end

	player:SetResource("GLIDER",cosmeticId)
	--ReliableEvents.BroadcastToAllPlayers("APPLYCOSMETIC", player.id, templateId)
end

function VerifyPurchase(player, cosmeticId)

	if StoreElements[cosmeticId] then
		if player:GetResource(StoreElements[cosmeticId][2]) >= StoreElements[cosmeticId][1] and not StoreElements[cosmeticId][3] then -- check if player can afford the item.
...
function BuyCosmetic(player, cosmeticId)
	if player and not Object.IsValid(player) or not player then
		return
	end
	
	if not VerifyPurchase(player, cosmeticId) then
	...
	if isPartOfSubscription then
		...
		player:SetResource(StoreElements[cosmeticId][2], player:GetResource(StoreElements[cosmeticId][2]) - StoreElements[cosmeticId][1])
	end
...
function GetCosmeticFromServer(player, cosmeticId)
    if not StoreElements[cosmeticId] then
        warn("Invalid cosmeticId: "..tostring(cosmeticId))
        return
    end
	...
	if data.COSMETICS then
		...
		if data.COSMETICS.owned then
			...
			if propAutosaveCurrency and data.COSMETICS.currency then
				...
				for c, r in pairs(Currencies) do
					if r and data and data.COSMETICS and data.COSMETICS.currency and data.COSMETICS.currency[c] then
