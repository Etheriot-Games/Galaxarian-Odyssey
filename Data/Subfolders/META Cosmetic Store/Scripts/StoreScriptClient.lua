...
local GLIDER_MODULE = require(script:GetCustomProperty('GliderModule'))
...
local propFirstPersonMode = propStoreRoot:GetCustomProperty('FirstPersonMode')


--Aj edited
local PREVIEW_POINT = script:GetCustomProperty('PreviewPoint'):WaitForObject()
local ShopOpen = false
local mapFromindexToStoreTemplateId = {}
------------------------------------------------------------------------------------------------------------------------
-- CUSTOM PROPERTIES
------------------------------------------------------------------------------------------------------------------------
...
local function MuteChildren(object)
    for key, value in pairs(object:GetChildren()) do
        MuteChildren(value)
        if value:IsA('Audio') or value:IsA('SmartAudio') then
            value:Destroy()
        end
    end
end
...
function ShowStore_ClientHelper()
    ...
    if localPlayer ~= Game.GetLocalPlayer() then
        return
    end
    ShopOpen = true
    ...
    pressedListener = localPlayer.bindingPressedEvent:Connect(OnRotateButtonPressed)
    releasedListener = localPlayer.bindingReleasedEvent:Connect(OnRotateButtonReleased)
...
function HideStore()
    ...
    ReliableEvents.BroadcastToServer('HIDESTORE_SERVER', localPlayer)
end
...
function StoreItemClicked(button)
    ...
    local currency = localPlayer:GetResource(currentlySelected.data.currencyName)
    ...
    elseif currentlySelected ~= nil then
        if HasCosmetic(currentlySelected.data.id) then
            ...
            if currentlySelected.PartOfSubscription and propAllowSubscriptionPurchase then
                if localPlayer:HasPerk(subscriptionPerkRef) then
                    expectedNewCurrency = currency
                    controlsLocked = true
                    while Events.BroadcastToServer('BUYCOSMETIC', currentlySelected.data.id) ==
...
function UpdateEntryButton(entry, highlighted)
    ...
    elseif not highlighted then
        ...
        local currency = localPlayer:GetResource(entry.data.currencyName)
...
function SpawnPreview(templateId, previewMesh, visible)
	...
	--]]
    previewMesh:MoveTo(PREVIEW_POINT:GetPosition(), 0.5, true)
    previewMesh:ScaleTo(Vector3.New(1, 1, 1), 0.5, true)
    previewMesh:RotateTo(Rotation.New(0, 0, -90), 0.5, true)
    ...
    if type(templateId) == 'number' then
        templateId = mapFromindexToStoreTemplateId[templateId]
    end
    local previewItem = World.SpawnAsset(templateId, {parent = previewMesh})

    local deco = previewItem:FindDescendantByName('store_graphic')
    deco.parent = previewMesh
    deco:SetPosition(Vector3.ZERO)
    --[[
...
function RemovePreview()
    for i, v in pairs(setPreviewMesh:GetChildren()) do
        v:Destroy()
    end
    previewElements = {}
    setPreviewMesh.visibility = Visibility.INHERIT
...
function HasCosmetic(storeId)
    if OwnedCosmetics[storeId] == true then
        ...
        return localPlayer:GetResource('COSMETIC_' .. storeId) > 0
    end
end
...
function ApplyCosmeticHelper(playerId, templateId)
	...
	for _, socket in pairs(playerSockets) do
		...
		if deco ~= nil then
			...
			if localPlayer.id == playerId and propFirstPersonMode and socket == "head" then
				deco.visibility = Visibility.FORCE_OFF
			end
		end
	end
...
function ClearList(direction)
    ...
    for k, v in pairs(StoreUIButtons) do
        ...
        v.targetPos =
            Vector3.New(v.gridX * -100, 0, v.gridY * -100) + Vector3.FORWARD * 1000 * direction + Vector3.UP * -300
...
function ForceDestroy(direction)
    if direction == nil then
        direction = 1
    end
    local startTime = time()
    ...
    for k = 1, ITEMS_PER_PAGE do
        ...
        local entry = {
            ...
            startPos = Vector3.New(gridX * -ITEM_PADDING, 0, gridY * -ITEM_PADDING) +
                Vector3.FORWARD * -1000 * direction +
                Vector3.UP * 300,
            targetPos = target,
            startTime = startTime,
...
function UpdateCurrencyDisplay(player, resource, amount)
    if Currencies[resource] then
        Currencies[resource].entryText.text = tostring(amount)
    end
end

function FullUpdateCurrencyDisplay()
    for k, v in pairs(Currencies) do
        v.entryText.text = tostring(localPlayer:GetResource(k))
    end
end
...
function InitStore()
    ...
    localPlayer.resourceChangedEvent:Connect(UpdateCurrencyDisplay)
    ...
    for k, v in pairs(propStoreContents:GetChildren()) do
        ...
        if storeInfo ~= nil then
            ...
            if tonumber(propID) then
                mapFromindexToStoreTemplateId[tonumber(propID)] = tempId
				Store_API.RegisterItem(propID,storeInfo:GetCustomProperty('SPAWN'))
            end
    ...
    if propEnableFilterByType then
        for k, v in ipairs(TypeList) do
            if v:sub(1, 1) ~= '_' then
                SpawnTypeFilterButton(
                    TypeDefs[v].name,
                    v,
                    TypeDefs[v].color,
                    count + TypeDefs[v].number,
...
function OnRotateButtonReleased(player, binding)
    ...
    if UI.GetCursorPosition().x > propRotateMarkerTopLeft.x and UI.GetCursorPosition().x < propRotateMarkerBottomRight.x then
        if
            UI.GetCursorPosition().y > propRotateMarkerTopLeft.y and
                UI.GetCursorPosition().y < propRotateMarkerBottomRight.y
         then
...
Events.Connect("GameStateChanged", function(oldState, newState, stateHasDuration, stateEndTime)
