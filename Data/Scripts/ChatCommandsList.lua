local ABI = require(script:GetCustomProperty("APIBasicInventory"))  
...
commands = {
    ...
    ['/money'] = {
        OnCommandCalledClient = function(player, message)
            ...
            Chat.LocalMessage('ERROR: command does not contain a valid resource amount. Format: /money <money>')
        end,
        OnCommandCalledServer = function(player, message)
            for section in (message .. ' '):gmatch('(.-) ') do
                if tonumber(section) then
                    player:SetResource('cash', tonumber(section))
        ...
        description = 'Set the cash resource of the player. Format: /money <money>',
        requireMessage = false,
        adminOnly = true,
        adminRank = AdminData.AdminRanks.HigherAdmin
    },
    ...
    ['/rocket'] = {
        ...
        OnCommandCalledServer = function(player, message)
            local newRocket =World.SpawnAsset(script:GetCustomProperty("LegendaryRocketLauncher"),{position = player:GetWorldPosition()})
            ABI.AddEquipment(player, newRocket)
        end,
        OnCommandReceivedClient = function(player, message)
        end,
        description = 'shows a list of available commands',
    ...
    ['/grenade'] = {
        ...
        OnCommandCalledServer = function(player, message)
            local newRocket =World.SpawnAsset(script:GetCustomProperty("nade"),{position = player:GetWorldPosition()})
            ABI.AddEquipment(player, newRocket)
        end,
        OnCommandReceivedClient = function(player, message)
        end,
