--[[
	...
	- GetObserversForSubject(subject)
...
function API.BeginStudy(observer, arguments)
	...
	-- This observer is not yet studying. Begin
		...
		-- Check if there are already existing observers. Set all observers to the same team
		if GetObserverCount() > 0 then
			for obs,_ in pairs(API.activeObservers) do
				observer.team = obs.team
				break
			end
		end
		...
		-- If observer is in a vehicle, destroy it
		local vehicle = observer.occupiedVehicle
...
function SetSubject(observer, subject)
	...
	-- Exit. Already studying this subject
	if data.subject == subject then return end
	...
	--if Object.IsValid(subject.occupiedVehicle) then
	--	API.HandlePlayerEnteredVehicle(subject, subject.occupiedVehicle)
	--else
		camera:AttachToPlayer(subject, CAMERA_ATTACHMENT_SOCKET)
	--end
	observer:SetPrivateNetworkedData("Observing", subject)
	if Environment.IsClient then 
		SpectateAPI.Spectate(subject)
	end
	-- Let other scripts and client know
...
function OnNetworkedPropertyChanged(obj, propertyName)
	if propertyName == "Subjects" then
		...
		if localWasSubject and not API.activeSubjects[localPlayer] then
			...
			SpectateAPI.Spectate(Game.GetLocalPlayer())
...
function API.RemoveDead(playerArray)
	for i = #playerArray, 1, -1 do
		if (playerArray[i].isDead ) then
			table.remove(playerArray, i)
		end
	end
	return playerArray
...
-- Server & Client
function API.GetObserversForSubject(subject)
	local result = {}
	if Environment.IsClient() then
		for _,observer in ipairs(Game.GetPlayers()) do
			if API.IsObserver(observer) then
				if API.GetSubjectForObserver(observer) == subject then
...
-- Server
function API.HandlePlayerEnteredVehicle(subject, vehicle)
	--[[ This is too complicated for a minor glitch fix. Disabling
	if API.IsSubject(subject) then
		-- For each observer's camera, attach to the vehicle instead of player
		for observer,_ in pairs(API.activeObservers) do
			local data = GetStudyData(observer)
			if data.subject == subject then
				print("Attaching cam to vehicle instead of subject")
				data.camera.parent = vehicle
				data.camera:SetPosition(Vector3.UP * 200)
				-- Handle case where vehicle is destroyed
				vehicle.destroyEvent:Connect(function(veh)
					if Object.IsValid(observer)
					and Object.IsValid(data.camera) and data.camera.parent == veh then
						-- In this case, the camera is going to be destroyed. Make another
						data.camera = World.SpawnAsset(SPECTATOR_CAMERA)
						data.camera:SetNetworkedCustomProperty("OwnerID", observer.id)
						data.camera:AttachToPlayer(subject, CAMERA_ATTACHMENT_SOCKET)
					end
				end)
				-- Handle case where the subject exits the vehicle
				local listener = vehicle.driverExitedEvent:Connect(function(veh, oldDriver)
					if Object.IsValid(oldDriver)
					and Object.IsValid(data.camera) and data.camera.parent == veh then
						-- Simply attach the camera back to the player
						data.camera:AttachToPlayer(oldDriver, CAMERA_ATTACHMENT_SOCKET)
					end
					-- Cleanup, because another player could take the vehicle
					data.driverExitedListener:Disconnect()
					data.driverExitedListener = nil
				end)
				data.driverExitedListener = listener
			end
		end
	end
	--]]
...
Game.playerJoinedEvent:Connect(function(player)
	player.diedEvent:Connect(function(p)
			-- If this player was being studied by another observer, tell them to study next player
			if API.IsSubject(p) then
				UpdateSubjectList()
				for obs,_ in pairs(API.activeObservers) do
					local _d = GetStudyData(obs)
					if _d.subject == p then
						API.NextSubject(obs)
					end
				end
			end
